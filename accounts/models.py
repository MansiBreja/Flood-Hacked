...
class allcoordinates(models.Model):
    coordinate=CharField(max_length=200,null=True)
    def set_coordinate(self, x):
        self.coordinate = json.dumps(x)

    def get_coordinate(self):
        return json.loads(self.coordinate)


class helicopdrones(models.Model):
    heli=models.IntegerField(null=False)
    drones=models.IntegerField(null=False)
